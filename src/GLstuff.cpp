        ...
        for (size_t h = 0; h < numHands; ++h) {
            ...
            if (numFingers >= 1) {
                ...
                for (size_t i = 0; i < numFingers; ++i) {
                    ...
                    float prevX = f->fX;
                    float prevY = f->fY;
                    float newX = ((finger.tipPosition().x + 200) / 400.f);// * GLStuff::gWidth;
                    float newY = ((finger.tipPosition().y - 150) / 200.f);// * GLStuff::gHeight;
                    f->SetX(newX);
                    f->SetY(newY);
                    f->scaleFactor = 1 - (finger.tipPosition().z / 300.f);
                    GLStuff::airMotion(newX, newY, finger.tipPosition().z, prevX , prevY);
